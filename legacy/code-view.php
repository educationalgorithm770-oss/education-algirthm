...
$stmt = $pdo->prepare("
    SELECT c.*, m.title as module_title, m.course_id, cs.title as course_title 
    FROM code_snippets c 
    JOIN modules m ON c.module_id = m.id 
    JOIN courses cs ON m.course_id = cs.id
    WHERE c.id = ?
");
$stmt->execute([$codeId]);
$snippet = $stmt->fetch();
...
if (!$snippet) {
    header("Location: " . ($isGuest ? "index.php" : "dashboard"));
    exit;
}

if (!$isGuest) {
    requireCourseAccess($studentId, $snippet['course_id']);
    $check = $pdo->prepare("SELECT id FROM lesson_completions WHERE student_id = ? AND item_type = 'code' AND item_id = ?");
    $check->execute([$studentId, $codeId]);
    $isComplete = (bool)$check->fetch();
} else {
    $isComplete = false;
}

$rawLang = strtolower(trim($snippet['language'] ?? 'java'));
if (empty($rawLang)) $rawLang = 'java';
$isWebLang = in_array($rawLang, ['html', 'htm', 'css', 'web'], true);
...
<head>
    ...
    <title><?php echo e($snippet["title"]); ?> — Multi-Language Code Studio • Education Algorithm</title>
...
<body data-page="code-view">
    ...
    <main class="app-container" style="max-width: 1280px; margin: 0 auto; padding: 1.25rem 1rem 4rem;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem; margin-bottom: 0.75rem;">
            <div>
                <a href="dashboard" style="color: #6366f1; text-decoration: none; font-size: 0.82rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.35rem; margin-bottom: 0.25rem;">
                    &larr; Back to Curriculum
                </a>
                <h1 style="font-size: 1.45rem; font-weight: 800; color: var(--text-primary); margin: 0;">
                    <?php echo e($snippet["title"]); ?>
                </h1>
                <p style="font-size: 0.82rem; color: var(--text-muted); margin: 0.15rem 0 0;">
                    Module: <?php echo e($snippet["module_title"]); ?> • Course: <?php echo e($snippet["course_title"]); ?> • Real-time Multi-Language Sandbox
                </p>
            </div>
            ...
            <form method="POST" action="mark-complete" style="margin: 0;">
                ...
                <input type="hidden" name="item_id" value="<?php echo $codeId; ?>">
                <button type="submit" class="btn <?php echo $isComplete ? 'btn-secondary' : 'btn-primary'; ?>" style="font-size: 0.84rem; padding: 0.45rem 1rem; border-radius: 8px;">
                    <?php echo $isComplete ? '✓ Completed' : 'Mark as Complete'; ?>
                </button>
            </form>
        </div>
...
a = 120
b = 80
print(f"Calculated Sum: {a + b}")
print("🚀 Real-time UTF-8 Emojis ✨")`,
            'html': `<!-- 🎨 Interactive Web GUI Sandbox -->
<div style="background: linear-gradient(135deg, #1e1b4b, #0f172a); border: 1px solid #4338ca; border-radius: 12px; padding: 24px; text-align: center; color: white; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.5);">
    <h2 style="margin-top: 0; color: #38bdf8; font-size: 1.35rem;">✨ Interactive GUI Sandbox</h2>
    <p style="color: #cbd5e1; font-size: 0.9rem;">Click the button below to test live JavaScript events and DOM reactivity!</p>
    <button onclick="document.getElementById('counter').innerText = parseInt(document.getElementById('counter').innerText) + 1" style="background: #4f46e5; color: white; border: none; padding: 10px 22px; font-size: 15px; font-weight: bold; border-radius: 8px; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(79,70,229,0.4);">
        🚀 Click Counter: <span id="counter" style="color: #34d399;">0</span>
    </button>
</div>`,
            'javascript': `const numbers = [10, 20, 30, 40];
const total = numbers.reduce((acc, curr) => acc + curr, 0);
console.log("JavaScript Total Sum:", total);`,
