...
<body>
    ...
    <?php
    $reqFeature = trim($_GET['feature'] ?? '');
    $reqSource  = trim($_GET['source'] ?? '');

    $featureTitles = [
        'curriculum'   => 'Curriculum & Video Modules',
        'doubts'       => '1-on-1 Faculty Doubt Desk',
        'flashcards'   => 'Interactive Flashcards',
        'certificates' => 'Verified Placement Certificates',
        'profile'      => 'Student LMS Dashboard',
        'support'      => 'Priority Student Helpdesk'
    ];
    $featureTitleText = $featureTitles[$reqFeature] ?? '';
    ?>

    <?php if (!empty($featureTitleText)): ?>
    <div style="background: linear-gradient(135deg, #1e1b4b, #312e81); border-bottom: 1px solid rgba(99, 102, 241, 0.4); padding: 0.75rem 1.25rem; color: #ffffff; text-align: center; font-size: 0.85rem; font-weight: 700; font-family: var(--font-heading); display: flex; align-items: center; justify-content: center; gap: 0.65rem; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
      <span style="background: #4f46e5; color: #fff; padding: 0.15rem 0.55rem; border-radius: 999px; font-size: 0.7rem; font-weight: 800;">🔒 UNLOCK FEATURE</span>
      <span>You requested access to <strong><?php echo htmlspecialchars($featureTitleText); ?></strong>. Complete your 2026 Cohort Enrollment below to unlock instant access!</span>
    </div>
    <?php endif; ?>
        ...
        <section class="flow-screen" id="screen2">
            <div class="dual-grid">
                ...
                <div class="master-card">
                    ...
                    <div class="otp-inputs-flex" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp0" autofocus oninput="handleOtpInput(this, 0)" onkeydown="handleOtpKey(event, 0)" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp1" oninput="handleOtpInput(this, 1)" onkeydown="handleOtpKey(event, 1)" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp2" oninput="handleOtpInput(this, 2)" onkeydown="handleOtpKey(event, 2)" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp3" oninput="handleOtpInput(this, 3)" onkeydown="handleOtpKey(event, 3)" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp4" oninput="handleOtpInput(this, 4)" onkeydown="handleOtpKey(event, 4)" onpaste="handleOtpPaste(event)">
                        <input type="text" maxlength="1" class="otp-box" id="otp5" oninput="handleOtpInput(this, 5)" onkeydown="handleOtpKey(event, 5)" onpaste="handleOtpPaste(event)">
                    </div>
            ...
            <div class="dual-grid">
                ...
                <aside class="master-card">
                    ...
                    <div class="price-lines-wrap">
                        ...
                        <div class="price-item-row" style="display: none;">
                            <span>GST</span>
    ...
    function applyPromoCode() {
        ...
        .then(data => {
            currentOrderId = '';
            if (data.success) {
                activeDiscountAmount = data.discount || 0;
                activeCouponText = data.code;
    ...
    function handleOtpPaste(e) {
        ...
        for (let i = 0; i < 6; i++) {
            const el = document.getElementById('otp' + i);
            if (el) el.value = pasteData[i] || '';
        }
        updateOtpFillStyle();
        const lastIdx = Math.min(pasteData.length - 1, 5);
        const focusTarget = document.getElementById('otp' + Math.min(lastIdx + 1, 5));
