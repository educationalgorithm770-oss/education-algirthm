...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["add_video"])) {
        ...
        if ($title && $moduleId > 0 && isset($_FILES["video_file"]) && $_FILES["video_file"]["error"] === UPLOAD_ERR_OK) {
            ...
            } elseif ($_FILES["video_file"]["size"] > 5120 * 1024 * 1024) {
                $error = "Video file size exceeds the 5GB limit.";
            } else {
...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .card {
        ...
        border-radius: 12px;
        padding: 1.25rem 1.4rem; margin-bottom: 2rem; box-shadow: 0 8px 24px rgba(0,0,0,0.4); }
    ...
    .form-label { display: block; font-size: 0.76rem; font-weight: 700; color: #94a3b8 !important; text-transform: uppercase; letter-spacing: 0.03em; margin-bottom: 0.35rem; }
...
<body>
    ...
    <div class="container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
            <div>
                <h1 style="font-size: 1.45rem; font-weight: 800; letter-spacing: -0.03em;">Curriculum & 3-Mode Video Studio</h1>
                <p style="color: var(--text-muted); font-size: 0.82rem; margin-top: 0.2rem;">Author video lectures, interactive code snippets, and PDF notes for your assigned tracks.</p>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;" class="grid-2">
            <!-- CARD 1: ADD MODULE -->
            <div class="card">
                <h3 style="font-size: 1.05rem; font-weight: 700; margin-bottom: 1.25rem;">1. Create Module / Chapter</h3>
            ...
            <div class="card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h3 style="font-size: 1.05rem; font-weight: 700; margin-bottom: 0;">🎬 2. Publish Video Lecture</h3>
                ...
                <div style="display: flex; gap: 0.5rem; background: #0b1120; border: 1px solid #1e293b; padding: 0.35rem; border-radius: 10px; margin-bottom: 1.25rem;">
                    <button type="button" id="btnTabDirect" onclick="switchVideoUploadMode('direct')" style="flex: 1; border: none; padding: 0.42rem 0.6rem; border-radius: 6px; font-weight: 700; font-size: 0.78rem; cursor: pointer; background: #6366f1; color: #fff;">⚡ Direct Cloud Upload</button>
                    <button type="button" id="btnTabGuid" onclick="switchVideoUploadMode('guid')" style="flex: 1; border: none; padding: 0.42rem 0.6rem; border-radius: 6px; font-weight: 700; font-size: 0.78rem; cursor: pointer; background: transparent; color: #94a3b8;">🔗 Link Stream GUID</button>
                    <button type="button" id="btnTabServer" onclick="switchVideoUploadMode('server')" style="flex: 1; border: none; padding: 0.42rem 0.6rem; border-radius: 6px; font-weight: 700; font-size: 0.78rem; cursor: pointer; background: transparent; color: #94a3b8;">💾 Server (Legacy)</button>
                </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;" class="grid-2">
            <!-- CARD 3: PDF NOTES -->
            <div class="card">
                <h3 style="font-size: 1.05rem; font-weight: 700; margin-bottom: 1.25rem;">📑 3. Attach PDF Lecture Notes</h3>
                <form method="POST" action="instructor-content.php" enctype="multipart/form-data">
                    <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            ...
            <div class="card">
                <h3 style="font-size: 1.05rem; font-weight: 700; margin-bottom: 1.25rem;">💻 4. Interactive Code Snippet</h3>
                <form method="POST" action="instructor-content.php">
                    <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
        ...
        <div class="card">
            <h3 style="font-size: 1.05rem; font-weight: 700; margin-bottom: 1.25rem;">Active Syllabus & Asset Tree (<?= count($modules) ?> Chapters)</h3>
                            ...
                            <div style="background: #141b2b; border: 1px solid var(--border-color); border-radius: 10px; padding: 0.85rem;">
                                ...
                                <?php else: ?>
                                    <?php foreach ($notes as $nt): ?>
                                        <div style="font-size: 0.82rem; padding: 0.35rem 0; border-bottom: 1px solid rgba(255,255,255,0.03); display: flex; justify-content: space-between; align-items: center;">
                                            <a href="<?= htmlspecialchars($nt['file_path']) ?>" target="_blank" rel="noopener noreferrer" style="color: #fbbf24; text-decoration: none;"><?= htmlspecialchars($nt['title']) ?></a>
                                            <form method="POST" action="instructor-content.php">
                                                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
